...
export default async function FavoritesPage({
	...
	const { data } = await supabase
		.from('favorites')
		.select(
			'listing_id, listings(id, slug, title, price, currency, deal_type, is_bulk, quantity, unit, age_months, created_at, status, listing_photos(path, order_index), regions(name_ru))',
		)
		.eq('user_id', user!.id)

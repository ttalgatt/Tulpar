...
-- Охват: 14 областей (данные по городам-регионам и новым областям 2022
-- добавляются вручную при необходимости)
-- =============================================================================
...
from (values
  ('astana-akmola', 'Астана', 'Астана', 'akmola'),
...
from (values
  ...
  ('aktobe-aktobe', 'Актобе', 'Актобе', 'aktobe'),
...
from (values
  ...
  ('konaev-almaty', 'Конаев', 'Конаев', 'almaty'),
  ...
  ('taldy-kurgan-almaty', 'Талды-Курган', 'Талды-Курган', 'almaty'),
...
select v.slug, v.name_ru, v.name_kk, r.id
from (values
  ('atyrau-atyrau', 'Атырау', 'Атырау', 'atyrau'),
...
from (values
  ...
  ('semey-east-kazakhstan', 'Семей', 'Семей', 'east-kazakhstan'),
  ...
  ('ust-kamenogorsk-east-kazakhstan', 'Усть-Каменогорск', 'Усть-Каменогорск', 'east-kazakhstan'),
...
from (values
  ...
  ('taraz-zhambyl', 'Тараз', 'Тараз', 'zhambyl'),
...
from (values
  ...
  ('ural-sk-west-kazakhstan', 'Уральск', 'Уральск', 'west-kazakhstan'),
...
from (values
  ...
  ('dzhezkazgan-karaganda', 'Джезказган', 'Джезказган', 'karaganda'),
  ('karaganda-karaganda', 'Караганда', 'Караганда', 'karaganda'),
  ...
  ('temirtau-karaganda', 'Темиртау', 'Темиртау', 'karaganda'),
...
from (values
  ...
  ('kostanay-kostanay', 'Костанай', 'Костанай', 'kostanay'),
...
from (values
  ...
  ('kyzylorda-kyzylorda', 'Кызылорда', 'Кызылорда', 'kyzylorda'),
...
from (values
  ...
  ('aktau-mangystau', 'Актау', 'Актау', 'mangystau'),
...
from (values
  ...
  ('pavlodar-pavlodar', 'Павлодар', 'Павлодар', 'pavlodar'),
  ('ekibastuz-pavlodar', 'Экибастуз', 'Экибастуз', 'pavlodar')
) as v(slug, name_ru, name_kk, region_slug)
join public.regions r on r.slug = v.region_slug
on conflict (region_id, slug) do nothing;
...
from (values
  ...
  ('petropavlovsk-north-kazakhstan', 'Петропавловск', 'Петропавловск', 'north-kazakhstan'),
...
from (values
  ...
  ('shymkent-chimkent-turkistan', 'Шымкент (Чимкент)', 'Шымкент (Чимкент)', 'turkistan'),
  ('chulakkurgan-turkistan', 'Чулаккурган', 'Чулаккурган', 'turkistan'),
  ('fogolevo-turkistan', 'Фоголево', 'Фоголево', 'turkistan'),
  ('kentau-turkistan', 'Кентау', 'Кентау', 'turkistan'),
  ...
  ('turkestan-turkistan', 'Туркестан', 'Туркестан', 'turkistan'),
